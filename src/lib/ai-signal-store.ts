...
export const upsertAiSignal = async (
  snapshot: AiSignalBase,
  options?: { explanation?: string; disclaimer?: string },
): Promise<void> => {
  const db = getDb();
  const explanation = options?.explanation ?? buildNarrative(snapshot);
  const disclaimer = options?.disclaimer ?? DISCLAIMER_COPY;
  ...
  `).run(
    ...
    snapshot.signal,
    snapshot.volEst,
    snapshot.onchainRisk,
    snapshot.onchainRiskLabel,
    snapshot.wNextConservador,
    snapshot.wNextBalanceado,
    snapshot.wNextAgresivo,
    snapshot.finalWConservador,
    snapshot.finalWBalanceado,
    snapshot.finalWAgresivo,
      explanation,
      disclaimer,
      JSON.stringify(null),
  );
